# KV caching in line with Meta implementation
# Hopefully this speeds up inference speed unlike previous debacle
# This kv caching scheme reduces 400 word inference time from 5.95 secs to 2.05 secs
# Use along with torch.compile for kv caching to work
...
def precompute_freq_cis(head_dim: int, seq_len: int, theta: float = 10000.0) -> Tuple[torch.Tensor, torch.Tensor]:
...
def apply_rotary_emb(xq: torch.Tensor, xk: torch.Tensor,
                     freqs_cos: torch.Tensor, freqs_sin: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        # pre-allocated KV cache: (B, L, n_kv_heads, head_dim)
    ...
    def forward(self, x: torch.Tensor, freqs_cos: torch.Tensor, freqs_sin: torch.Tensor, start_pos: int) -> torch.Tensor:
        ...
        # during inference, write into the pre-allocated cache
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int = 0) -> torch.Tensor:
        ...
        for layer in self.layers:
            h = layer(h, self.freqs_cos, self.freqs_sin, start_pos)
        h = self.norm(h)
        out = self.output(h)
        return out
